...
def voxelize(dm_halo, gas_halo, nvoxel):
    ...
    gas_pos, gas_mass = dm_halo[:, 0:3], dm_halo[:, 3]
    ...
    dm_coalesced = torch.sparse_coo_tensor(indices=dm_pos.T, values=dm_mass, size=(nvoxel, nvoxel, nvoxel)).coalesce()
    gas_coalesced = torch.sparse_coo_tensor(indices=gas_pos.T, values=gas_mass, size=(nvoxel, nvoxel, nvoxel)).coalesce()
...
def preprocess(opt):
    data_sim_path = Path(opt.data_path) / f"{opt.sim_name}_SNAP{opt.snap_num:03d}_MASS{opt.mass_min:.2e}_{opt.mass_max:.2e}_NGASMIN{opt.n_gas_min}"
    dataset_sim_path = Path(opt.dataset_path) / opt.sim_name
    for mode in ("train", "valid", "test"):
        (data_sim_path / f"nvoxel_{opt.nvoxel}" / mode).mkdir(parents=True, exist_ok=True)
    ...
    hsmall = il.groupcat.loadHeader(str(dataset_sim_path / "output"), opt.snap_num)["HubbleParam"]
    lbox = il.groupcat.loadHeader(str(dataset_sim_path / "output"), opt.snap_num)["BoxSize"]  # kpc/h
    mdm = {
        "TNG300-1": 4e7 / hsmall,
    ...
    try:
        ...
        ids = find_ids(base_path=str(dataset_sim_path / "output"), hsmall=hsmall, snap_num=opt.snap_num, mass_min=opt.mass_min, mass_max=opt.mass_max, n_gas_min=opt.n_gas_min)
        np.save(str(data_sim_path / "ids.npy"), ids)

    ready_dm = list((data_sim_path / f"nvoxel_{opt.nvoxel}").glob("*/halo_*_dm.npy"))
    ready_gas = list((data_sim_path / f"nvoxel_{opt.nvoxel}").glob("*/halo_*_gas.npy"))
    ...
    if len(ids) == len(ready_dm) == len(ready_gas) > 0:
        ...
        for i, halo_id in enumerate(ids):
            dm_halo, gas_halo = find_halo(base_path=str(dataset_sim_path / "output"), halo_id=halo_id, lbox=lbox, mdm=mdm, snap_num=opt.snap_num)
            ...
            torch.save(dm_coalesced, data_sim_path / f"nvoxel_{opt.nvoxel}" / mode / f"halo_{halo_id}_dm_coalesced.npy")
            torch.save(gas_coalesced, data_sim_path / f"nvoxel_{opt.nvoxel}" / mode / f"halo_{halo_id}_gas_coalesced.npy")
