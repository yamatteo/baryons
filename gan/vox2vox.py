...
def single_run(opt: Namespace):
    ...
    for epoch in range(start_from_epoch, opt.n_epochs):
        ...
        for i, (real_dm, real_gas) in enumerate(vv.dataloaders[opt.train_mode]):  # batch is a dictionary
            ...
            logging.info(f"Inputs and model %% {torch.cuda.memory_allocated():_d}")
            ...
            logging.info(f"generator(...) %% {torch.cuda.memory_allocated():_d}")


            loss_gen = vv.discriminator.evaluate(real_dm, real_gas, pred_gas)

            logging.info(f"discriminator.evaluate(...) %% {torch.cuda.memory_allocated():_d}")

            loss_gen.backward()

            logging.info(f"loss_gen.backward() %% {torch.cuda.memory_allocated():_d}")
