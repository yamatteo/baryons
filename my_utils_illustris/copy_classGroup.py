...
class Group:
    ...
    def __init__(self, path, snap, simname, ID):
        ...
        if self.part[5]>0:
           ...
           BHpos_shifted = myil.utils.get_periodic_coords(self.center, BHpos, self.Lbox)
           BHmasses      = il.snapshot.loadHalo(path, snap, ID, 'bh', fields=['Masses'])
    ...
    def M_delta(self, delta_goal, start_radius):
        ...
        pos  = np.concatenate((self.dmpos_shifted, self.gaspos_shifted, self.starpos_shifted), axis=0)                         # kpc/h
        mass = np.concatenate((np.ones(len(self.dmpos_shifted))*self.mdm, self.gasmasses*1e10, self.starmasses*1e10), axis=0)  # Msun/h
